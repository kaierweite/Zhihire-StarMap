...
@Slf4j
@Service
public class ApplyInviteServiceImpl implements ApplyInviteService {
    ...
    public ApplyInviteServiceImpl(RecommendRecordMapper recommendRecordMapper, ResumeMapper resumeMapper,
                                  JobMapper jobMapper, UserMapper userMapper, CompanyMapper companyMapper,
    ...
    public void applyJob(Long userId, Long jobId) {
        Resume resume = resumeMapper.selectOne(new LambdaQueryWrapper<Resume>().eq(Resume::getUserId, userId).last("LIMIT 1"));
        ...
        RecommendRecord record = recommendRecordMapper.selectOne(new LambdaQueryWrapper<RecommendRecord>()
                .eq(RecommendRecord::getUserId, userId).eq(RecommendRecord::getJobId, jobId));
        if (record == null) {
            record = new RecommendRecord(); record.setUserId(userId); record.setJobId(jobId);
            record.setIsClicked(true); record.setIsApplied(true); record.setIsInvited(false);
            recommendRecordMapper.insert(record);
        } else { record.setIsApplied(true); recommendRecordMapper.updateById(record); }
        Company company = companyMapper.selectById(job.getCompanyId());
        if (company != null) {
            User applicant = userMapper.selectById(userId);
            notificationService.createNotification(company.getUserId(), "收到新投递",
                    String.format("求职者 %s 投递了岗位「%s」", applicant != null ? applicant.getUsername() : "未知", job.getTitle()), "APPLICATION");
        }
    ...
    public void inviteTalent(Long userId, Long resumeId, Long jobId) {
        ...
        if (company == null || !job.getCompanyId().equals(company.getId())) throw new BusinessException(403, "无权操作此岗位");
        RecommendRecord record = recommendRecordMapper.selectOne(new LambdaQueryWrapper<RecommendRecord>()
                .eq(RecommendRecord::getUserId, resume.getUserId()).eq(RecommendRecord::getJobId, jobId));
        if (record == null) {
            record = new RecommendRecord(); record.setUserId(resume.getUserId()); record.setJobId(jobId);
            record.setIsClicked(false); record.setIsApplied(false); record.setIsInvited(true);
            recommendRecordMapper.insert(record);
        } else { record.setIsInvited(true); recommendRecordMapper.updateById(record); }
        notificationService.createNotification(resume.getUserId(), "面试邀请",
                String.format("企业「%s」邀请您面试岗位「%s」", company.getCompanyName(), job.getTitle()), "INTERVIEW_INVITE");
        log.info("面试邀请：resumeId={}, jobId={}", resumeId, jobId);
    }

    @Override
    public List<RecommendRecord> getInvitations(Long userId) {
        return recommendRecordMapper.selectList(new LambdaQueryWrapper<RecommendRecord>()
                .eq(RecommendRecord::getUserId, userId).eq(RecommendRecord::getIsInvited, true).orderByDesc(RecommendRecord::getCreatedAt));
    }
}