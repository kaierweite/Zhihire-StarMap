...
public class SystemFileService {
    ...
    @Value("${starmap.upload.base-path:data/starmap/files}")
    ...
    public SystemFileService(UploadFileMapper uploadFileMapper) {
    ...
    public UploadFile store(MultipartFile file, Long uploaderId) {
        ...
        if (!ALLOWED_EXTENSIONS.contains(ext.toLowerCase())) {
            throw new BusinessException(400, "仅支持 PDF/DOC/DOCX 格式");
        }
        ...
        try {
            ...
            try (InputStream is = file.getInputStream()) {
                Files.copy(is, storagePath, StandardCopyOption.REPLACE_EXISTING);
            }
        ...
        uploadFile.setPath(storagePath.toString());
    ...
    private void validateMagicNumber(MultipartFile file, String ext) {
        try {
            ...
            if (allBytes.length < 4) throw new BusinessException(400, "文件过小");
            byte[] header = Arrays.copyOfRange(allBytes, 0, 4);
            String lowerExt = ext.toLowerCase();
            if (".pdf".equals(lowerExt) && !startsWith(header, PDF_MAGIC)) {
                throw new BusinessException(400, "PDF 文件魔数校验失败");
            }
            if (".docx".equals(lowerExt) && !startsWith(header, ZIP_MAGIC)) {
                throw new BusinessException(400, "DOCX 文件魔数校验失败");
            }
