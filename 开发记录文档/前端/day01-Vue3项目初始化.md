...
- [x] 安装核心依赖
  ...
  ```
- [x] 配置 Element Plus 按需导入（unplugin-vue-components）
- [x] 配置 vite.config.ts（别名 @ → src、代理 /api → localhost:8080）
- [x] 创建目录结构：api / views / router / store / components / styles / utils
- [x] 创建全局样式 styles/global.scss（导入设计令牌 colors）
- [x] 创建 Axios 封装 utils/request.ts（请求拦截器加 token、响应拦截器统一错误处理）
- [x] 创建 App.vue 基础布局
